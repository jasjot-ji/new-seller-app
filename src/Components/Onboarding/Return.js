  ...
  render() {
    ...
    const addDetails = () => {
      if (this.state.state === '' || this.state.town_city === '' || this.state.flat_house_building === '' || this.state.area === '' || this.state.landmark === '' || this.state.pincode === '' || this.state.address_type === '') {
        ...
        fetch(`${baseUrl}/add_return_address`, requestOptions)
          .then(response => response.json())
          .then(result => {
            if (result.status === 'success') {
              this.props.alert('Added Return Address', 'success');
    ...
    const useAddress = () => {
      ...
      var raw = JSON.stringify({
        sellers_address_id: shipping_address.data.sellers_address_id,
        return_default_address: true,
      });
    ...
    return (
      <ScrollView style={{paddingHorizontal: 20, marginTop: 30}}>
        <View style={styles.innerContainer}>
          <Text style={styles.heading}>Add your Return Address</Text>
          <Pressable
            onPress={() => {
              this.setState({
                verifyButtonDisabled: false,
                ...
                town_city: shipping_address.data.town_city,
                flat_house_building: shipping_address.data.flat_house_building,
                area: shipping_address.data.area,
                landmark: shipping_address.data.landmark,
                pincode: shipping_address.data.pincode,
                address_type: shipping_address.data.address_type,
                return_default_address: true,
              });
            }}>
